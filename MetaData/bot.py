...
• Доступно сегодня: `{stats.get('remaining', 0)}` из `{stats.get('daily_limit', 3)}`
...
📅 **Сегодня:**
• Использовано: `{stats.get('daily_used', 0)}` из `{stats.get('daily_limit', 3)}`
...
🎁 **Бонусы:**
• За рефералов: `+{stats.get('referrals', {}).get('active', 0)}` запросов сегодня
"""
...
📊 **Лимиты:**
• 3 запроса в день
• +1 запрос за приглашенного друга
...
def main() -> None:
    ...
    print(f"📊 Лимит: 3 запроса в день")
